...
async function editSubscription(subscriptionId) {
    try {
        const response = await fetch(`/get_subscription/${subscriptionId}`);
        const subscription = await response.json();

        elements.serviceSelect.value = subscription.service_name;
        elements.nextPaymentDate.value = subscription.start_date;
        elements.amount.value = subscription.amount;
        elements.currency.value = subscription.currency;
        elements.bank.value = subscription.bank || '';
        elements.cardLast4.value = subscription.card_last_4 || '';
        elements.sendNotifications.checked = subscription.send_notifications;

        onServiceChange();
        showModal('Редактировать подписку');
        currentSlide = 1;
        showSlide(currentSlide);

        tg.MainButton.setText('Сохранить');
        tg.MainButton.show();
        tg.MainButton.onClick(() => saveSubscription(subscriptionId));
    } catch (error) {
        console.error('Error fetching subscription details:', error);
        tg.showPopup({message: 'Ошибка при загрузке данных подписки'});
    }
}
...
function showCalendar() {
    debugLog('Отображение календаря');
    document.querySelector('.container').innerHTML = '<div id="calendarContainer">Loading calendar...</div>';
    fetch('/calendar')
        .then(response => {
            debugLog('Получен ответ от сервера с календарем');
            return response.text();
        })
        .then(html => {
            debugLog('Получен HTML календаря');
            document.getElementById('calendarContainer').innerHTML = html;
            debugLog('Инициализация календаря');
            initializeCalendar();
        })
        .catch(error => debugLog(`Ошибка при загрузке календаря: ${error.message}`));
}
...
function initializeCalendar() {
    ...
    function renderCalendar(subscriptionDates) {
    ...
    function showSubscriptions(day, subscriptions) {
        // Реализуйте эту функцию для отображения подписок на конкретный день
        debugLog(`Showing subscriptions for day ${day}`);
    }
